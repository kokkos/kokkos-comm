...
template <typename T>
class IAllReduce : public testing::Test {
 public:
  using Scalar = T;
};

using ScalarTypes = ::testing::Types<int, int64_t, float, double, Kokkos::complex<float>, Kokkos::complex<double>>;
TYPED_TEST_SUITE(IAllReduce, ScalarTypes);

TYPED_TEST(IAllReduce, 1D_contig) {
  ...
  auto request = comm.iallreduce(sendv, recvv, sum);
  request.wait();
