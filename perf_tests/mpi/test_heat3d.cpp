...
struct SystemKC {
  // Communicator
  CommHelper comm;
  std::vector<KokkosComm::Request<>> mpi_requests;
  ...
  void exchange_T_halo() {
    ...
    if (X_lo != 0) {
      auto h =
          KokkosComm::Communicator<KokkosComm::MpiSpace, Kokkos::DefaultExecutionSpace>::from_raw(comm.comm, E_left);
      mpi_requests.push_back(KokkosComm::recv(h, T_left, comm.left));
      mpi_requests.push_back(KokkosComm::send(h, T_left_out, comm.left));
    }
    if (Y_lo != 0) {
      auto h =
          KokkosComm::Communicator<KokkosComm::MpiSpace, Kokkos::DefaultExecutionSpace>::from_raw(comm.comm, E_down);
      mpi_requests.push_back(KokkosComm::recv(h, T_down, comm.down));
      mpi_requests.push_back(KokkosComm::send(h, T_down_out, comm.down));
    }
    if (Z_lo != 0) {
      auto h =
          KokkosComm::Communicator<KokkosComm::MpiSpace, Kokkos::DefaultExecutionSpace>::from_raw(comm.comm, E_front);
      mpi_requests.push_back(KokkosComm::recv(h, T_front, comm.front));
      mpi_requests.push_back(KokkosComm::send(h, T_front_out, comm.front));
    }
    if (X_hi != X) {
      auto h =
          KokkosComm::Communicator<KokkosComm::MpiSpace, Kokkos::DefaultExecutionSpace>::from_raw(comm.comm, E_right);
      mpi_requests.push_back(KokkosComm::recv(h, T_right, comm.right));
      mpi_requests.push_back(KokkosComm::send(h, T_right_out, comm.right));
    }
    if (Y_hi != Y) {
      auto h = KokkosComm::Communicator<KokkosComm::MpiSpace, Kokkos::DefaultExecutionSpace>::from_raw(comm.comm, E_up);
      mpi_requests.push_back(KokkosComm::recv(h, T_up, comm.up));
      mpi_requests.push_back(KokkosComm::send(h, T_up_out, comm.up));
    }
    if (Z_hi != Z) {
      auto h =
          KokkosComm::Communicator<KokkosComm::MpiSpace, Kokkos::DefaultExecutionSpace>::from_raw(comm.comm, E_back);
      mpi_requests.push_back(KokkosComm::recv(h, T_back, comm.back));
      mpi_requests.push_back(KokkosComm::send(h, T_back_out, comm.back));
    }
  }

  void compute_surface_dT() {
    KokkosComm::wait_all(mpi_requests);
