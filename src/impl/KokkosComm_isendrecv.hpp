...
Request isendrecv(SendView sv, RecvView rv, int rank, int tag, Communicator comm) {
  ...
  if (not SVT::is_contiguous(sv) or not RVT::is_contiguous(rv))
    throw std::runtime_error("only contiguous views supported for low-level isendrecv");

  auto req = comm.isendrecv(sv, rv, rank, tag);
