...
template <>
class Communicator<Experimental::RcclSpace, Kokkos::HIP> {
 public:
  ...
  /// @brief Constructs a `Communicator` from a raw `ncclComm_t` handle and a Kokkos HIP execution space instance.
  /// Defaults `exec` to `Kokkos::HIP`.
  ///
  ...
  /// @brief Splits from a raw RCCL communicator by color and key.
  ///
  ...
  /// @brief Duplicates from a raw RCCL communicator.
