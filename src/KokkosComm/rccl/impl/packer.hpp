...
struct PackedRcclView {
  View view_;
  ncclDataType_t datatype_;
  int count_;

  PackedRcclView(const View& view, ncclDataType_t datatype, int count)
...
struct DeepCopy {
  ...
  static auto allocate_packed_for(const E& exec, const std::string& label, const V& view) -> PackedRcclView<PackedV> {
    auto packed = KokkosComm::Impl::allocate_contiguous_for(exec, label, view);
    return PackedRcclView<PackedV>(packed, datatype<RcclSpace, T>(), span(packed));
  }
  ...
  template <KokkosExecutionSpace E>
  static auto pack(const E& exec, const std::string& label, const V& view) -> PackedRcclView<PackedV> {
